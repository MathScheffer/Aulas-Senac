    ...
    conexao.query(sql,[body.nome],(error,rows) => {
        if(error){
            ...
            if(Utils.retornaVariasLinhas(rows,"nome",body.nome).length == 0 ){
                if(rows.length == 1){
                    let check =  bcrypt.compare(body.senha,rows[0].senha);
                    //console.log(bcrypt.compareSync(body.senha,rows[0].senha))
                    check.then(response => {
                        if(response){
                            console.log(body.senha,rows[0].senha, response)
                            var token = jwt.sign({ Nome: body.nome,Senha: rows[0].senha }, 'shhhhh',{expiresIn:'1h'});
    ...
    params.forEach((param, index) => {
        if(bodyObj.has(param) && bodyObj.get(param)){ 
            ...
            if(param === "senha"){
                ...
                console.log("Senha enviada: ",bodyObj.get(param))
                console.log("Valor do hash criado: ",value);
                console.log('Resultado:',bcrypt.compareSync(bodyObj.get(param),value))
            }
            values.push(value);
        }
    })
